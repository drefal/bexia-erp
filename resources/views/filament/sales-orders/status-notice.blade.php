@php
    ...
    if (! empty($saleOrderId) && Schema::hasTable('sales_orders')) {
        $order = DB::table('sales_orders')->where('id', (int) $saleOrderId)->first();

        if ($order && class_exists(\App\Support\SalesApprovalWorkflow::class)) {
            $notice = \App\Support\SalesApprovalWorkflow::salesOrderWorkflowNotice($order);
        }
    }
