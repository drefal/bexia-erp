    ...
    <div class="space-y-6">
        <div class="rounded-xl border border-gray-200 bg-white shadow-sm">
            ...
            <div class="space-y-5 p-6">
                <div class="flex flex-wrap gap-3" style="display:flex;gap:10px;flex-wrap:wrap;align-items:center;">
                    <a
                        href="{{ route('account-receivables.reports.customer-statement.pdf', [
                            'tenant' => $tenantId,
                            'date_from' => $dateFrom,
                            'date_to' => $dateTo,
                    ...
                    <a
                        href="{{ route('account-receivables.reports.customer-statement.excel', [
                            'tenant' => $tenantId,
                            'date_from' => $dateFrom,
                            'date_to' => $dateTo,
                            'customer_contact_id' => $customerContactId,
                ...
                <form method="GET" class="grid gap-4 md:grid-cols-4">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            placeholder="Buscar CxC o cobro"
                            class="mt-1 block w-full rounded-lg border-gray-300 px-3 py-2 shadow-sm"
                        >
                    </div>

                    <div class="md:col-span-4 flex flex-wrap gap-3 pt-1">
                        <button
                            type="submit"
        ...
        <div class="grid gap-4 md:grid-cols-4">
            <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                ...
                <div class="mt-2 text-lg font-bold">${{ number_format($totals['receivables_total'], 2) }} MXN</div>
            </div>
            <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                <div class="text-xs text-gray-500">Cobrado en documentos</div>
                <div class="mt-2 text-lg font-bold">${{ number_format($totals['collected_total'], 2) }} MXN</div>
            </div>
            <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                <div class="text-xs text-gray-500">Saldo pendiente</div>
                <div class="mt-2 text-lg font-bold">${{ number_format($totals['balance_total'], 2) }} MXN</div>
            </div>
            <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                <div class="text-xs text-gray-500">Cobros aplicados periodo</div>
                <div class="mt-2 text-lg font-bold">${{ number_format($totals['payments_total'], 2) }} MXN</div>
            </div>
        </div>

        <div class="rounded-xl border border-gray-200 bg-white shadow-sm">
            <div class="border-b border-gray-200 px-6 py-4 font-semibold text-gray-900">Documentos CxC</div>
                    ...
                    <tbody>
                        @forelse($receivables as $row)
                            <tr class="border-b last:border-b-0 hover:bg-gray-50">
                                ...
                                <td class="px-4 py-3">{{ $row->issue_date ?: '-' }}</td>
                                <td class="px-4 py-3">{{ $row->due_date ?: '-' }}</td>
                                <td class="px-4 py-3">{{ \App\Filament\Resources\AccountReceivableResource::statusLabel($row->status) }}</td>
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="px-4 py-6 text-center text-gray-500">Sin documentos en el periodo.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div class="rounded-xl border border-gray-200 bg-white shadow-sm">
            <div class="border-b border-gray-200 px-6 py-4 font-semibold text-gray-900">Cobros</div>
                ...
                <table class="w-full border-collapse text-sm">
                    <thead>
                        <tr class="border-b text-left text-xs uppercase tracking-wide text-gray-500">
                            ...
                            <th class="px-4 py-3">Fecha</th>
                            <th class="px-4 py-3">Cliente</th>
                            <th class="px-4 py-3">CxC</th>
                            <th class="px-4 py-3">Estado</th>
                            <th class="px-4 py-3">Póliza</th>
                    ...
                    <tbody>
                        @forelse($payments as $row)
                            <tr class="border-b last:border-b-0 hover:bg-gray-50">
                                ...
                                <td class="px-4 py-3">{{ $row->customer_name ?: 'Sin cliente' }}</td>
                                <td class="px-4 py-3">{{ $row->receivable_number }}</td>
                                <td class="px-4 py-3">{{ \App\Filament\Resources\AccountReceivablePaymentResource::statusLabel($row->status) }}</td>
                                <td class="px-4 py-3">{{ $row->accounting_entry_number ?: ($row->accounting_entry_id ?: 'Sin póliza') }}</td>
                                <td class="px-4 py-3">{{ $row->reference ?: '-' }}</td>
                                <td class="px-4 py-3 text-right font-semibold">${{ number_format((float) $row->amount, 2) }} {{ $row->currency }}</td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="8" class="px-4 py-6 text-center text-gray-500">Sin cobros en el periodo.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
