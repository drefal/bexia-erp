...
class EditStockSerialNumber extends EditRecord
{
    protected static string $resource = StockSerialNumberResource::class;

    protected function getSavedNotificationTitle(): ?string
    {
        return 'Número de serie actualizado correctamente';
    }

    protected function mutateFormDataBeforeSave(array $data): array
    {
        return $this->validateSerialData($data, (int) $this->record->getKey());
    }
    ...
    protected function validateSerialData(array $data, ?int $ignoreId = null): array
    {
        ...
        if ($productId <= 0) {
            $this->notifyError('No se pudo guardar la serie', 'Selecciona un producto.');
            throw ValidationException::withMessages(['product_id' => 'Selecciona un producto.']);
        }

        if ($serialNumber === '') {
            $this->notifyError('No se pudo guardar la serie', 'Captura el número de serie.');
        ...
        if (! $product) {
            $this->notifyError('No se pudo guardar la serie', 'El producto seleccionado no existe.');
