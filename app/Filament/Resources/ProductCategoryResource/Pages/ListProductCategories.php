...
class ListProductCategories extends ListRecords
{
    ...
    {
        return [
            \Filament\Actions\Action::make('export_product_categories_xlsx')
                ->label('Exportar Excel')
                ->icon('heroicon-o-arrow-down-tray')
                ->color('success')
                ->action(fn () => \App\Support\ProductCategoryCatalogExportService::downloadCategoriesXlsx()),

            \Filament\Actions\Action::make('download_product_categories_template')
                ->label('Plantilla carga masiva')
                ->icon('heroicon-o-table-cells')
                ->color('warning')
                ->action(fn () => \App\Support\ProductCategoryCatalogExportService::downloadTemplateXlsx()),

            \Filament\Actions\Action::make('import_product_categories')
                ->label('Importar categorías')
                ->icon('heroicon-o-arrow-up-tray')
                ->color('primary')
                ->modalWidth('5xl')
                ->modalHeading('Importar categorías de productos')
                ...
                ->form([
                    \Filament\Forms\Components\FileUpload::make('file')
                        ...
                        ->disk('local')
                        ->directory('imports/categorias-productos')
                        ->visibility('private')
                        ->preserveFilenames()
                        ->acceptedFileTypes([
                        ...
                        ->afterStateUpdated(function ($state, \Filament\Forms\Set $set): void {
                            ...
                            $validation = \App\Support\ProductCategoryCatalogImportService::validateForModalFromFilamentUpload($state);
                ...
                ->action(fn (array $data) => \App\Support\ProductCategoryCatalogImportService::importValidatedModalUpload(
                    $data['file'] ?? null,
