...
return new class extends Migration
{
    public function up(): void
    {
        /*
         * IMPORTANTE:
         * Bexia ya cuenta con catálogo SAT de unidades.
         * Esta migración NO crea ni llena sat_units.
         * Solo asegura la tabla de equivalencias de compra por producto.
         */
        ...
        if (Schema::hasTable('purchase_request_lines')) {
            Schema::table('purchase_request_lines', function (Blueprint $table) {
                if (! Schema::hasColumn('purchase_request_lines', 'sat_unit_key')) {
                    $table->string('sat_unit_key', 20)->nullable()->index();
                }

                if (! Schema::hasColumn('purchase_request_lines', 'sat_unit_name')) {
                    $table->string('sat_unit_name', 150)->nullable();
                }
            });
        }
        ...
        DB::table('product_purchase_units')
            ->whereNull('sat_unit_key')
            ->whereRaw("LOWER(name) IN ('pieza', 'pza', 'piezas')")
            ->update([
                'sat_unit_key' => 'H87',
                'sat_unit_name' => 'Pieza',
            ]);
        ...
        DB::table('product_purchase_units')
            ->whereNull('sat_unit_key')
            ->whereRaw("LOWER(name) LIKE '%paquete%'")
            ->update([
                'sat_unit_key' => 'XPK',
                'sat_unit_name' => 'Paquete',
            ]);

        DB::table('product_purchase_units')
            ->whereNull('sat_unit_key')
            ->whereRaw("LOWER(name) LIKE '%docena%'")
            ->update([
                'sat_unit_key' => 'DPC',
                'sat_unit_name' => 'Docenas de piezas',
            ]);
    }
    ...
    public function down(): void
    {
        if (Schema::hasTable('purchase_request_lines')) {
            Schema::table('purchase_request_lines', function (Blueprint $table) {
                foreach (['sat_unit_key', 'sat_unit_name'] as $column) {
