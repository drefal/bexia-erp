...
return new class extends Migration
{
    ...
    {
        if (! Schema::hasTable('employee_payroll_perceptions')) {
            Schema::create('employee_payroll_perceptions', function (Blueprint $table): void {
                $table->id();
                $table->foreignId('company_id')->constrained()->cascadeOnDelete();
                $table->foreignId('employee_id')->constrained('employees')->cascadeOnDelete();
                $table->foreignId('payroll_concept_id')->nullable()->constrained('payroll_concepts')->nullOnDelete();

                $table->string('type', 40)->default('bonus')->index();
        ...
        if (! Schema::hasTable('employee_payroll_perception_applications')) {
            Schema::create('employee_payroll_perception_applications', function (Blueprint $table): void {
                $table->id();
                $table->foreignId('company_id')->constrained()->cascadeOnDelete();
                $table->foreignId('employee_payroll_perception_id')->constrained('employee_payroll_perceptions')->cascadeOnDelete();
                ...
                $table->unique(['employee_payroll_perception_id', 'payroll_run_id'], 'employee_perception_run_unique');
    ...
    {
        Schema::dropIfExists('employee_payroll_perception_applications');
        Schema::dropIfExists('employee_payroll_perceptions');
    }
};
